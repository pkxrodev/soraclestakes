...
python3.10 -m venv SoracleStakes_venv
source SoracleStakes_venv/bin/activate
pip install --upgrade pip
git clone https://github.com/pkxrodev/SoracleStakes
cd nado
pip install -r requirements.txt
```

To go back to your screen, use `screen -r SoracleStakes` 
To update your SORACLESTAKES installation, use 
...
#### Direct installation
First [download](https://github.com/pkxrodev/SoracleStakes/archive/refs/heads/main.zip) the master branch from GitHub and extract the archive.
Run the command line as Administrator and enter the following commands:
```
python -m pip install -r requirements.txt
```

# To run SoracleStakes, execute the following command: `python3.10 SoracleStakes.py`
...
--recipient, <SoracleStakes address> Recipient address
...
## Private key storage 
- Linux: `/~/SoracleStakes/private/keys.dat`

To view your file, you can use the following command: `cat ~/SoracleStakes/private/keys.dat`. You may need to exit screen first using `CTRL+a` and then `CTRL+d`.

- Windows: `C:\Users\[username]\SoracleStakes\private`
...
## What is SoracleStakes?

<p align="center">
  <img src="https://nodeisok.com/media/overview.png" />
</p>

SoracleStakes. It is just another blockchain written from scratch with a highly experimental consensus algorithm, 
...
## What's the reason for SoracleStakes?

SoracleStakes is a take on one of the newer trends, where users do not use graphics cards or specialized hardware for mining, nor
...
## What does SoracleStakes do differently?

In SoracleStakes, every user generates new blocks at the same time. This is possible because users are not rewarded for producing
...
the network. SoracleStakes prefers decentralization to efficiency, which means it exposes consensus more than the individual
node, which makes it more resilient regarding SPOF but less efficient.

## But is there a premise?

The premise of SoracleStakes is that users stop being interested in decentralized value distributing projects because it becomes
...
in the success of SoracleStakes, where no single entity was capable of obtaining a high amount of tokens through monopolizing 
token distribution. 
...
of 2014. SoracleStakes puts these promises into practice. 
...
## Proof of what?
Every node in the SoracleStakes ecosystem keeps track of what opinions other nodes have by sharing state checksums for current
...
For rewards, SoracleStakes uses a variant of PoW with a very limited input set unlike in normal PoW crypto, where it is unlimited.
...
User chance to be rewarded for a block is random and depends on their address. This is because of how SoracleStakes is designed,
...
are counted from the smallest unit of SoracleStakes, so the lowest penalty resolution is 0.0000000001 and the lowest burn/bribe 
...
To burn your SoracleStakes, send it to the following address: `burn`


## What about security?

There are no guarantees for security of SoracleStakes, mainly because of its highly experimental character. Compared to more
...
When implementing new functionalities to SoracleStakes, existing routines/loops should be used instead of instant invocation of functions.
